...
export default async function ProductPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const usesRecipe = ["MIX", "COMBO", "ELABORADO"].includes(product.product_kind);
    ...
    <div className="grid min-w-0 gap-4 xl:grid-cols-[1.45fr_.8fr]">
      <div className="min-w-0 space-y-4">
        <section className="pt-card overflow-hidden">
          <div className="border-b border-[#f2e0e8] px-4 py-4 sm:px-5"><p className="pt-section-title">Datos del producto</p><h2 className="mt-1 font-black text-[#3e2833]">Presentación comercial: {saleUnit}</h2><p className="mt-0.5 text-xs text-[#896b78]">SKU, precio, costo comercial y peso de envío. La variante técnica de Tiendanube queda oculta.</p></div>
          {primaryVariant ? canEdit ? <form action={updateSaleOption} className="grid gap-4 p-4 sm:grid-cols-2 sm:p-5">
            ...
            <div className="flex items-end"><button className="pt-button-primary w-full px-4">Guardar cambios</button></div>
      ...
      <aside className="min-w-0 space-y-4">
        <section className="pt-card overflow-hidden">
          ...
          <div className="p-4 sm:p-5"><h2 className="font-black text-[#3e2833]">Información general</h2>{canEdit ? <form action={updateProduct} className="mt-4 space-y-3"><input type="hidden" name="product_id" value={product.id} /><Field label="Nombre"><input className="pt-input" name="name" defaultValue={product.name} required /></Field><div className="grid grid-cols-2 gap-3"><Field label="Código"><input className="pt-input" name="code" defaultValue={product.code ?? ""} /></Field><Field label="Categoría comercial"><input className="pt-input" name="category" defaultValue={product.category ?? ""} /></Field></div><div className="grid grid-cols-2 gap-3"><Field label="Tipo de producto"><select className="pt-input" name="product_kind" defaultValue={product.product_kind}><option value="INSUMO">Producto simple / comprado</option><option value="MIX">Mix con receta</option><option value="ELABORADO">Elaborado con receta</option><option value="COMBO">Combo</option></select></Field><Field label="Control de stock"><select className="pt-input" name="inventory_mode" defaultValue={product.inventory_mode}><option value="PROPIO">Stock propio</option><option value="DERIVADO">Derivado de receta</option><option value="PRODUCIDO">Producido previamente</option></select></Field></div><Field label="Descripción"><textarea className="pt-input min-h-24 resize-y" name="description" defaultValue={product.description ?? ""} /></Field><Field label="URL de imagen"><input className="pt-input" type="url" name="image_url" defaultValue={product.image_url ?? ""} placeholder="https://…" /></Field><div className="grid grid-cols-2 gap-3"><Field label={`Stock mínimo (${product.base_unit === "g" ? "g" : "u"})`}><input className="pt-input" type="number" min="0" step="1" name="minimum_stock" defaultValue={String(product.minimum_stock ?? 0)} /></Field><Field label="Costo base"><input className="pt-input" type="number" min="0" step="0.0001" name="current_cost" defaultValue={String(product.current_cost ?? 0)} /></Field></div><div className="flex flex-wrap gap-4"><Check name="active" label="Activo" checked={product.active} /><Check name="visible" label="Visible" checked={product.visible} /><Check name="published" label="Publicado" checked={product.published} /></div><button className="pt-button-primary w-full px-4">Guardar cambios</button></form> : <p className="mt-4 text-xs text-[#80616f]">Solo ADMIN puede editar la ficha.</p>}</div>
        </section>
